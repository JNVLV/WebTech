...
regForm.addEventListener("submit", (event) => {
  ...
  if (!surname || !name || !time || !control) {
    alert(
      `Пожалуйста, заполните поле ${!surname ? "Фамилия " : ""}${
        !name ? (!surname ? "Имя " : ", Имя") : ""
      }${!time ? (!surname && !name ? "Время " : ", Время") : ""}${
        !control
          ? !surname && !name && !time
            ? "Форма контроля"
            : ", Форма контроля"
  ...
  const examSubject = document.querySelector(
    `#select-lesson option[value="${lesson}"]`
  ).textContent;
  const examTime = time;
